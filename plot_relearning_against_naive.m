...
%%
% analysis_groups = [1, 3, 5];
% analysis_groups = [1 6 7 8];
...
%         asymDir(i_sub, i_grp) = nanmean(asym_array) - (CORRECT_BASELINE)*nanmean(baseline_array);
    ...
    figure; hold on;
    plot(1:N_TRIALS_TO_SHOW, nanmean(relearnCurves(:,1:N_TRIALS_TO_SHOW),1), [grp_clrs{analysis_groups(i_grp)}]);
    plot(1:N_TRIALS_TO_SHOW, nanmean(relearnCurves(:,1:N_TRIALS_TO_SHOW),1) + sqrt(nanvar(relearnCurves(:,1:N_TRIALS_TO_SHOW),[],1)./sum(~isnan(relearnCurves(:,1:N_TRIALS_TO_SHOW)),1)), [grp_clrs{analysis_groups(i_grp)}]);
    plot(1:N_TRIALS_TO_SHOW, nanmean(relearnCurves(:,1:N_TRIALS_TO_SHOW),1) - sqrt(nanvar(relearnCurves(:,1:N_TRIALS_TO_SHOW),[],1)./sum(~isnan(relearnCurves(:,1:N_TRIALS_TO_SHOW)),1)), [grp_clrs{analysis_groups(i_grp)}]);
    axis([0 N_TRIALS_TO_SHOW -5 35])
end
